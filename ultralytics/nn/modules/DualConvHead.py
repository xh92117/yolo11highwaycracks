...
class Detect_DualConvHead(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):
        ...
        # 修改 cv2 分支（使用 DualConv 替换 Conv）
        self.cv2 = nn.ModuleList(
            nn.Sequential(
                DualConv(x, c2, stride=1, g=2),    # 替换第一个 Conv(x, c2, 3)
                DualConv(c2, c2, stride=1, g=2),   # 替换第二个 Conv(c2, c2, 3)
                nn.Conv2d(c2, 4 * self.reg_max, 1)  # 最后一层保持不变
            ) for x in ch
        )
        # 修改 cv3 分支（使用 DualConv 替换 DWConv 和 Conv）
        self.cv3 = nn.ModuleList(
            nn.Sequential(
                nn.Sequential(
                    DualConv(x, x, stride=1, g=2),  # 替换 DWConv(x, x, 3)
                    DualConv(x, c3, stride=1, g=2)  # 替换 Conv(x, c3, 1)
                ),
                nn.Sequential(
                    DualConv(c3, c3, stride=1, g=2),  # 替换 DWConv(c3, c3, 3)
                    DualConv(c3, c3, stride=1, g=2)   # 替换 Conv(c3, c3, 1)
                ),
                nn.Conv2d(c3, self.nc, 1)  # 最后一层保持不变
            ) for x in ch
        )
