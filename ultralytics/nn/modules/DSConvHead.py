...
###################### DSConv  ####     END   by  AI&CV  ###############################
class Detect_DSConvHead(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):
        ...
        self.cv2 = nn.ModuleList(
            nn.Sequential(
                DSConv2D(x, c2, 3),      # 原Conv -> DSConv2D
                DSConv2D(c2, c2, 3),     # 原Conv -> DSConv2D 
                nn.Conv2d(c2, 4 * self.reg_max, 1)
            ) for x in ch
        )
        # 分类分支cv3（保留DWConv，替换后续普通Conv）
        self.cv3 = nn.ModuleList(
            nn.Sequential(
                nn.Sequential(
                    DWConv(x, x, 3),     # 原DWConv保持不变
                    DSConv2D(x, c3, 1)    # 原Conv -> DSConv2D
                ),
                nn.Sequential(
                    DWConv(c3, c3, 3),   # 原DWConv保持不变
                    DSConv2D(c3, c3, 1)   # 原Conv -> DSConv2D
                ),
